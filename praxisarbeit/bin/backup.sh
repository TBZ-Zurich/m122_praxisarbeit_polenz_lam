    ...
    BACKUP='etc/userwizzard/backup._etc-example_.config'
    DONTBACKUP='etc/userwizzard/dontbackup._etc-example_.config'
    DIR="${CWD}/etc/userwizzard/"
    KEEPINDAYS=1
    FILENAME='backup'
    ...
    read_backup() {
        ...
        for i in etc/userwizzard/backup.*.config; do
            ...
            echo 'Filename '$i >> $log
            echo 'Amount of matching config file '$BACKUPCOUNTER >> $log
...
check_amount() {
    ...
    find $BACKUPLOCATION -mtime +$KEEPINDAYS -type f -delete 
